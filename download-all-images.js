...
function getExtension(url) {
  ...
  // Try to detect from content-type later
  return ".jpg";
}
...
async function processAll() {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    if (imageUrl.startsWith("/")) {
      console.log(`  ✅ [${i + 1}/${products.length}] ${product.name} — Ya es local`);
      skipped++;
      continue;
    }
    ...
    if (imageUrl.startsWith("data:")) {
      console.log(`  ⏭️  [${i + 1}/${products.length}] ${product.name} — Es data URI`);
      skipped++;
      continue;
    }
    ...
    // Check if file already exists (to skip re-download)
    if (existingFiles.has(fileName.toLowerCase()) || existingFiles.has(fileName.toLowerCase())) {
      console.log(`  📁 [${i + 1}/${products.length}] ${product.name} — Ya existe en disco`);
      // Still need to update the catalog path
    ...
    try {
      ...
      // Add to existing files set to avoid checking again
